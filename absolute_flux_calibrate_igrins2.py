#Script to generate absolute flux calibrated IGRINS-2 data and output
#it as a .flux_a0v.fits file
...
date = sys.argv[1] #Date to process is a command line argument
...
slit_length = 5 #length of slit in arcsec
guiding_error = 0 #Presumeably Gemini North has good guiding
		...
		if len(query_coords) != 0:
			...
			for i in range(0, len(query_coords)):
				...
				if main_id_tg in ''.join(std_star_name.split()):
					...
					print(f"\033[38;5;{196}mONLY ONE OBJECT FOUND THROUGH COORDINATE SEARCH, ASSUMING STANDARD IS {query_coords['main_id'][0]}\033[0m")
			...
			if std_star_name == std_star_name_0:
				print(f'\033[38;5;{196}m{std_star_name} IS NOT SIMBAD SEARCHABLE AND WE CANNOT MATCH THE COORDINATES TO AN OBJECT IN SIMBAD RELIABLY')
...
for spec_a0v_file_path in spec_a0v_file_paths:
	print('Processing '+spec_a0v_file_path)
	...
	tgt_obsid = int(spec_a0v_hdul[4].header['ORIGNAME'][10:14]) #Note IGRINS-2 specific naming format
	#grab the obsid for the standard	
	std_obsid = int(spec_a0v_hdul[6].header['ORIGNAME'][10:14])
	...
	try:
		...
		std_ra = spec_a0v_hdul[6].header['RA']
		#grabbing the standard star DEC
		std_dec = spec_a0v_hdul[6].header['DEC']
	...
	tgt_pa = spec_a0v_hdul[4].header['PA']
	#grabbing the standard position angle
	std_pa = spec_a0v_hdul[6].header['PA']

	#grabbing the target exposure time
	tgt_exptime = spec_a0v_hdul[4].header['EXPTIME']
	#grabbing the standard exposure time
	std_exptime =  spec_a0v_hdul[6].header['EXPTIME']
	...
	tgt_total_on_time = tgt_exptime * tgt_nexp #Calculate total time on target and std
	...
	print(f'\033[38;5;{79}m\nESTIMATING TARGET SLIT THROUGHPUT\033[0m')
	...
	if 'STELLAR' in recipe_log['RECIPE'][tgt_indx]: #Only apply throughput correction for stellar targets
		...
		tgt_throughput, tgt_m, tgt_b, tgt_fluxcorr, tgt_fluxcorr_m, tgt_fluxcorr_b = tgt_spec.getSlitThroughput(name=tgt_name, name_prefix='Target:', slit_length=slit_length, PA=tgt_pa, guiding_error=guiding_error, plot=True, pdfobj=pdfobj, nod_off_slit=tgt_nod_off_slit) #Calculate slit throughput for target and standard
		# tgt_throughput, tgt_m, tgt_b = tgt_spec.getSlitThroughput(name=tgt_name, name_prefix='Target:', slit_length=slit_length, PA=tgt_pa, guiding_error=guiding_error, plot=True, pdfobj=pdfobj, nod_off_slit=tgt_nod_off_slit) #Calculate slit throughput for target and standard
		#divide target spec by the throughput (normalize by the throughput)
		tgt_spec = tgt_spec / tgt_throughput
		#Apply flux correction
		#tgt_spec = tgt_spec * tgt_fluxcorr
		print(f'\033[38;5;{79}mTHE MEDIAN TARGET FLUX CORRECTION IS {(np.nanmedian(tgt_fluxcorr)-1)*100:.2f} PERCENT\033[0m')
	...
	if std_filename != last_std_filename: #If standard has changed
		...
		std_throughput, std_m, std_b, std_fluxcorr, std_fluxcorr_m, std_fluxcorr_b = std_spec.getSlitThroughput(name=std_star_name, name_prefix='Standard Star:', slit_length=slit_length, PA=std_pa, guiding_error=guiding_error, plot=True, pdfobj=pdfobj, nod_off_slit=std_nod_off_slit)
		# std_throughput, std_m, std_b = std_spec.getSlitThroughput(name=std_star_name, name_prefix='Standard Star:', slit_length=slit_length, PA=std_pa, guiding_error=guiding_error, plot=True, pdfobj=pdfobj, nod_off_slit=std_nod_off_slit)
		#divide standard spec by the throughput (normalize by the throughput)
		std_spec = std_spec / std_throughput
		# #Apply flux correction
		# std_spec = std_spec * std_fluxcorr
		print(f'\033[38;5;{33}mTHE MEDIAN STANDARD FLUX CORRECTION IS {(np.nanmedian(std_fluxcorr)-1)*100:.2f} PERCENT\033[0m')
	...
	flattened_tgt_spec_flux_calibrated = tgt_spec_flux_calibrated.trim_overlap().remove_nans().remove_outliers(threshold=20).stitch()
	...
	for band in ['H', 'K']:
		...
		std_model_arr = resampled_std_model.get_plp_array(band=band, kind='flux') #Generate 2D array for standard star best fit model flux
		
		#Open hdul object for spec_a0v.fits file
		hdul = fits.open(filepath + filename) #Open hdul object for spec_a0v.fits file
